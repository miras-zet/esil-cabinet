...
export function AnketaKz() {
  ...
  const seriya = `№${applicationData?.certificate_serial != '' ? applicationData?.certificate_serial : applicationData?.alternative_certificate_serial ? applicationData?.alternative_certificate_serial:'_______'} ${applicationData?.certificate_number != '' ? applicationData?.certificate_number : applicationData?.alternative_certificate_number ? applicationData?.alternative_certificate_number:'______________'}, ${applicationData?.alternative_certificate_date!='' ? moment(applicationData?.alternative_certificate_date).format("LL") :  moment(applicationData?.certificate_date).format("LL")} ${applicationData?.altynBelgi!==0?'(Алтын белгі)':''}`;
  const anketaValuesRu = [
    ...
    ['Отбасы жағдайы', applicationData?.ismarried==1 ? 'Үйленген':'Үйленбеген'],
    ...
    ['Жеңілдіктер', applicationData?.benefits],
