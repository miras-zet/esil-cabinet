...
import { StatementDOTResponse } from '../../models/response/StatementDOTResponse';

export function StatementDOTRu() {

    const data = JSON.parse(localStorage.getItem('data'));
    const [statementData, setStatementData] = useState<StatementDOTResponse>();
    moment.locale('ru');
    useEffect(() => {
        DocsService.getStatementDOTDataRu().then((response) => {
    ...
    const generatePdf = () => {
        ...
        var opt = {
            ...
            html2canvas: { scale: 2 },
            filename: `Заявление_ДОТ_Рус_${localStorage.getItem('currentApplicantFIO')}.pdf`,
